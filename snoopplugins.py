...
wZ1bad = []  #filtered ip (not ip) or filtered Yandex data, filtered 'geocoordinates'
azS = []  #list of future request results
coord = []  #coordinates multi-purpose list
...
## ERR.
def Erf(hvostfile):
    print(f"\033[31;1mCannot find_read file: '{hvostfile}'.\033[0m \033[36m\n " + \
          f"\nPlease provide a text file encoded in —\033[0m \033[36;1mutf-8.\033[0m\n" + \
          f"\033[36mBy default, for example, notepad in OS Windows saves text in encoding — ANSI.\033[0m\n" + \
...
# Parsing.
            ...
            with progressYa:
                ...
                for reqY, login in zip(azS, listlogin):
                    ...
                    try:
                        ...
                        if rdict.get('title') == "404 Not Found":  #equals
                            raise Exception("")
                    except Exception:
                        rdict = {}
                    ...
                    if rdict.get("display_name") == "-No-":
                        ...
                        table1 = Table(title=f"\n{Style.DIM}{Fore.CYAN}results{Style.RESET_ALL}", style="green")
                        ...
                        print("\033[32;1mYa.Zen:\033[0m", dzen)
        ...
        print("\n\033[36m[\033[0m\033[32;1m1\033[0m\033[36m] --> Specify user login\n" + \
              "[\033[0m\033[32;1m2\033[0m\033[36m] --> Specify public link to Yandex.Disk\n" + \
              "[\033[0m\033[32;1m3\033[0m\033[36m] --> Specify user ID\n" + \
...
# Specify login.
            ...
            print("\033[36m└──Enter login/email of the user, e.g.,\033[0m\033[32;1m bobbimonov\033[0m\n")
...
# Specify Yandex.Disk link.
            ...
            print("\033[36m└──Enter public link to Yandex.Disk, e.g.,\033[0m\033[32;1m https://yadi.sk/d/7C6Z9q_Ds1wXkw\033[0m\n")
...
# Specify Yandex user ID.
        elif Ya == '3':
            print("\033[36m└──Enter Yandex user ID, e.g.,\033[0m\033[32;1m tr6r2c8ea4tvdt3xmpy5atuwg0\033[0m\n")
            login = console.input("[cyan]hash --->  [/cyan]")
            listlogin.append(login)

            if len(login) != 26:
                print(Style.BRIGHT + Fore.RED + "└──Incorrect user ID" + Style.RESET_ALL)
                ravno()
            else:
                parsingYa()
...
# Specify file with logins.
            ...
            print("\033[31;1m└──This plugin method is not yet implemented\033[0m\n")
        else:
...
def module2():
    if hasattr(sys, 'getandroidapilevel'):
        print(Style.BRIGHT + Fore.RED + "└──Reverse Vgeocoder plugin is 'complex' and not supported (by default) " + \
              "in Snoopy for Termux\n\nExit\n" + Style.RESET_ALL)
        sys.exit()
    while True:
        print("""
...
# select file with geocoordinates.
            ...
            while True:
                print("\033[36m└──Enter \033[0m\033[32;1mabsolute path\033[0m \033[36mto the file (file encoding -> utf-8) with data: \n\
        [geo-coordinates] or drag and drop file into terminal window\033[0m\n")
...
# Check coordinate file path.
                try:
                    if os.path.exists(put) is False:
                        raise Exception("")
                    break
                except Exception:
                    print("\033[31;1m└──Invalid path. " + \
                          "Provide a correct absolute path to object or drag file into terminal window\033[0m")
                    hvostput = os.path.split(put)[1].replace('"', '')
                    Erf(hvostput)

            while True:
                print("\n\033[36m╭Choose geocoding mode:\033[0m\n" + \
                      "\033[36m├──\033[36m[\033[0m\033[32;1m1\033[0m\033[36m] --> Simple \033[0m\n" + \
                      "\033[36m├──\033[36m[\033[0m\033[32;1m2\033[0m\033[36m] --> Detailed \033[0m\n" + \
...
# Threads.
    def treads_dr(fun, args, dr):
        """Threads are needed to wrap built-in socket functions that do not have a timeout.

        In case of slowdown (on Android, waiting can reach > 1 minute, usually an empty result is returned,
        when ip/domain is false) destroy threads after a given time 't_socket'.
        """
...
# Select single search or '-f'.
    ravno()
    print("\n\033[36mEnter domain (example:\033[0m \033[32;1mexample.com\033[0m\033[36m),\n" + \
          ...
          "[\033[0m\033[32;1mfile\033[0m\033[36m] --> data file processing\n" + \
          "[\033[0m\033[32;1menter\033[0m\033[36m] --> information about own GEO_IP\n" + \
          "[\033[0m\033[31;1mq\033[0m\033[36m] --> Exit")
...
# Invalid key choice during offline/online search. Exit.
            else:
                print(Style.BRIGHT + Fore.RED + "└──Invalid choice" + Style.RESET_ALL)
                ravno()
